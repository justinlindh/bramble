...
bool probe_reply_queue_earliest_due(const pending_probe_reply_t *q, int cap, uint32_t *earliest_due_ms) {
    bool have = false;
    uint32_t earliest = 0;
    for (int i = 0; i < cap; i++) {
        if (!q[i].used) continue;
        if (!have || q[i].due_at_ms < earliest) { earliest = q[i].due_at_ms; have = true; }
    }
    if (have && earliest_due_ms) *earliest_due_ms = earliest;
    return have;
}

int probe_reply_queue_find_due(const pending_probe_reply_t *q, int cap, uint32_t now_ms) {
    for (int i = 0; i < cap; i++) {
        if (q[i].used && q[i].due_at_ms <= now_ms) return i;
    }
    return -1;
}

void probe_reply_queue_apply_result(pending_probe_reply_t *item, probe_reply_tx_result_t result, uint32_t now_ms) {
