...
void handle_delivery_receipt(const uint8_t* data, uint8_t len, int16_t rssi, int8_t snr) {
     ...
     * and by which route. The unicast ACK path logs its equivalent ("Message
     * delivered to %08X" above), while this path logged nothing at all: the
     * receipt only reached a connected RPC client through
     * mesh_emit_broadcast_delivery_notification, so on a bench node with no
     * client attached (and in the emulator, which carries no RPC transport)
     * the arrival of a delivery receipt was invisible. The relay path is
