...
size_t delivery_event_ring_deserialize(delivery_event_ring_t* ring, const uint8_t* in,
    ...
    if (hdr.count > hdr.capacity || hdr.write_index >= hdr.capacity || hdr.next_seq == 0u) {
        return 0u;
    }

    bytes_needed = sizeof(hdr) + (size_t)hdr.capacity * sizeof(delivery_event_record_t);
    if (in_len < bytes_needed) {
        return 0u;
    }
