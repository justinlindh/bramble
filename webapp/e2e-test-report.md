...
But when `loadMessages()` fetches the same message from firmware, it arrives with the real node address as `from` (e.g., `0x1A2B3C4D`).

`isLikelyDuplicate()` checks `existing.from !== candidate.from` (line 339), which returns `false` because `0 !== 0x1A2B3C4D`, so the message is **not recognized as a duplicate** and gets added again.
...
### Map Tab
- Node markers shown with friendly names (Ridge, Trailhead, Downtown, Northside, E2E-Test) ✅
