...
describe('device save on connect', () => {
  it('formats a numeric address to 8-char uppercase hex', () => {
    expect(formatAddrHex(0xF2BE6EEE)).toBe('F2BE6EEE');
  ...
  it('saveConnectedDevice upserts by hex address and stores the token per remember', () => {
    saveConnectedDevice({ addr: 0xF2BE6EEE, name: 'V4', ip: '192.168.2.146', token: 'tok', remember: true, transport: 'wifi' });
    const d = listDevices();
    expect(d).toHaveLength(1);
    expect(d[0].address).toBe('F2BE6EEE');
    expect(d[0].lastIp).toBe('192.168.2.146');
    expect(getDeviceToken('F2BE6EEE')).toBe('tok');
    expect(localStorage.getItem('bramble.deviceToken.F2BE6EEE')).toBe('tok'); // remembered
  });
