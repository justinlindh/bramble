  ...
  rpcMock.mockImplementation(async (method: string) => {
    switch (method) {
      ...
      case 'bramble.getConfig':
        return { identity: { address: 0xf2be6eee } };
...
describe('android native bridge notification on connect', () => {
  it('hands wsUrl and token to the bridge after a wifi connect in the Android shell', async () => {
    ...
    await connect('wifi', { url: 'ws://192.168.1.21/ws', token: 'sekrit', ip: '192.168.1.21' });

    expect(useStore.getState().connectionState).toBe('connected');
    expect(updateConnection).toHaveBeenCalledWith('ws://192.168.1.21/ws', 'sekrit');
  });
  ...
  it('passes an empty token when none is set', async () => {
    ...
    await connect('wifi', { url: 'ws://192.168.1.21/ws', ip: '192.168.1.21' });

    expect(updateConnection).toHaveBeenCalledWith('ws://192.168.1.21/ws', '');
  });
