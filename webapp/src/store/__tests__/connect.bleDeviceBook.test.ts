...
beforeEach(() => {
  ...
  rpcMock.mockImplementation(async (method: string) => {
    switch (method) {
      ...
      case 'bramble.getConfig': return { identity: { address: 0xf2be6eee } };
      case 'bramble.getStatus': return { address: 'F2BE6EEE' };
...
describe('BLE device book', () => {
  it('saves the node with a remembered token after a BLE connect', async () => {
    await connect('ble', { token: 'ble-secret', remember: true, name: 'Garage' });
    expect(useStore.getState().connectionState).toBe('connected');
    const saved = listDevices().find(d => d.transport === 'ble');
    expect(saved?.address).toBe('F2BE6EEE');
    expect(saved?.name).toBe('Garage');
    expect(getDeviceToken('F2BE6EEE')).toBe('ble-secret');
    // remembered -> persisted to localStorage, not just the session
    expect(localStorage.getItem('bramble.deviceToken.F2BE6EEE')).toBe('ble-secret');
  });
  ...
  it('keeps the BLE token session-only when Remember is off', async () => {
    ...
    expect(getDeviceToken('F2BE6EEE')).toBe('ble-secret');
    expect(localStorage.getItem('bramble.deviceToken.F2BE6EEE')).toBeNull();
  });
});
