...
describe('incoming message native notification', () => {
  ...
  it('learns the sender name from fromName so an unknown peer never titles as hex', async () => {
    ...
    await deliver({ from: 'AABBCC04', to: 'AA11', text: 'hi', msgId: 'm7', fromName: 'Anthem' });
    expect(onMessage).toHaveBeenCalledTimes(1);
    const payload = JSON.parse(onMessage.mock.calls[0][0]);
    expect(payload).toMatchObject({ sender: 'Anthem', conversationTitle: 'Anthem' });
    expect(useStore.getState().peerNames.get(0xaabbcc04)).toBe('Anthem');
  });
