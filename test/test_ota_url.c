...
void test_origin_https_with_port_is_valid(void) {
    TEST_ASSERT_TRUE(ota_url_origin_valid("https://203.0.113.34:8443/fw/", false));
}

void test_origin_http_rejected_unless_allowed(void) {
    TEST_ASSERT_FALSE(ota_url_origin_valid("http://203.0.113.34:8088/", false));
    TEST_ASSERT_TRUE(ota_url_origin_valid("http://203.0.113.34:8088/", true));
}
...
void test_resolve_rejects_bad_origin(void) {
    ...
    TEST_ASSERT_EQUAL_INT(
        OTA_URL_ERR_ORIGIN,
        ota_url_resolve("http://203.0.113.34:8088/", "bramble.bin", false, out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("", out);
}

void test_resolve_allows_http_origin_in_dev_mode(void) {
    char out[OTA_URL_MAX];
    TEST_ASSERT_EQUAL_INT(OTA_URL_OK, ota_url_resolve("http://203.0.113.34:8088", "bramble.bin",
                                                      true, out, sizeof(out)));
    TEST_ASSERT_EQUAL_STRING("http://203.0.113.34:8088/bramble.bin", out);
}
