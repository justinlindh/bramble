...
void test_queue_insert_uses_free_slot(void) {
    ...
    int slot = probe_reply_queue_insert(q, PROBE_REPLY_QUEUE_CAPACITY, buf, 6, PROBE_REPLY_ATTEMPTS,
                                        5000u);
...
void test_queue_insert_full_returns_negative(void) {
    ...
    for (int i = 0; i < PROBE_REPLY_QUEUE_CAPACITY; i++)
        TEST_ASSERT_TRUE(probe_reply_queue_insert(q, PROBE_REPLY_QUEUE_CAPACITY, buf, 1, 3, 100u) >=
                         0);
