...
static int h_share_location_once(const cJSON* params, cJSON* result) {
    ...
    cJSON_AddStringToObject(result, "tier",
                            tier && tier->valuestring ? tier->valuestring : "coarse");
    return 0;
}
...
void test_send_message_missing_params_returns_invalid_params(void) {
    ...
    char response[512];
    int len = rpc_dispatch("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"bramble.sendMessage\","
                           "\"params\":{\"dest\":\"ABCDEF01\"}}",
...
void test_set_radio_out_of_range_returns_invalid_params(void) {
    ...
    char response[512];
    int len = rpc_dispatch("{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"bramble.setRadio\","
                           "\"params\":{\"tx_power\":99}}",
                           response, sizeof(response));
    TEST_ASSERT_GREATER_THAN(0, len);
...
void test_handler_radio_error_bubbles_to_error_response(void) {
    ...
    int len = rpc_dispatch(
        "{\"jsonrpc\":\"2.0\",\"id\":3,\"method\":\"bramble.sendProbe\",\"params\":{}}", response,
        sizeof(response));
    TEST_ASSERT_GREATER_THAN(0, len);
...
void test_set_location_config_accepts_hybrid_policy_fields(void) {
    ...
    int len = rpc_dispatch(
        ...
        "\"contact_rules\":[{\"address\":\"AABBCCDD\",\"enabled\":true,\"tier\":\"full\","
        "\"interval_s\":120}],"
        "\"channel_targets\":[{\"channel\":0,\"enabled\":true,\"tier\":\"coarse\",\"interval_s\":"
        "600}]}}",
        response, sizeof(response));
    TEST_ASSERT_GREATER_THAN(0, len);
...
void test_share_location_once_reports_location_packet_type(void) {
    ...
    char response[512];
    int len = rpc_dispatch("{\"jsonrpc\":\"2.0\",\"id\":7,\"method\":\"bramble.shareLocationOnce\","
                           "\"params\":{\"address\":\"AABBCCDD\"}}",
                           response, sizeof(response));
    TEST_ASSERT_GREATER_THAN(0, len);

    cJSON* resp = parse_response(response);
    cJSON* result = cJSON_GetObjectItem(resp, "result");
    TEST_ASSERT_EQUAL_STRING("PKT_TYPE_LOCATION",
                             cJSON_GetObjectItem(result, "packetType")->valuestring);
    cJSON_Delete(resp);
}
...
void test_share_location_once_honors_requested_tier(void) {
    ...
    char response[512];
    int len = rpc_dispatch("{\"jsonrpc\":\"2.0\",\"id\":8,\"method\":\"bramble.shareLocationOnce\","
                           "\"params\":{\"address\":\"AABBCCDD\",\"tier\":\"presence\"}}",
                           response, sizeof(response));
    TEST_ASSERT_GREATER_THAN(0, len);
