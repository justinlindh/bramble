...
static int mock_write(uint64_t v, void* ctx) {
    (void)ctx;
    if (g_write_should_fail) return -1; /* not durable: g_flash must NOT change */
    g_flash = v; g_writes++; return 0;
}

void setUp(void) { g_flash = 0; g_writes = 0; g_write_should_fail = 0; }
void tearDown(void) {}
...
void test_reboot_resumes_above_ceiling(void) {
    TEST_ASSERT_EQUAL(0, nonce_counter_init(1, 0, mock_read, mock_write, NULL));   /* flash -> 65536 */
    uint8_t n[12];
