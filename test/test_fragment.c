...
void test_reassembly_duplicate(void) {
    ...
    int rc = reassembly_add(&ctx, &hdr, data, 10, 1000);
    TEST_ASSERT_EQUAL(0, rc);
    /* Add same fragment again */
    rc = reassembly_add(&ctx, &hdr, data, 10, 1000);
...
void test_reassembly_timeout(void) {
    ...
    uint8_t data[10] = {0};
    reassembly_add(&ctx, &hdr, data, 10, 1000);
    ...
    hdr.frag_index = 1;
    int rc = reassembly_add(&ctx, &hdr, data, 10, 32000);
...
void test_reassembly_concurrent(void) {
    ...
    for (int m = 0; m < 4; m++) {
        ...
        int rc = reassembly_add(&ctx, &hdr, data, 10, 5000);
        TEST_ASSERT_EQUAL(0, rc);
    }
    ...
    uint8_t data5[10] = {0};
    int rc = reassembly_add(&ctx, &hdr5, data5, 10, 5000);
    ...
    rc = reassembly_add(&ctx, &hdr_fin, data_fin, 10, 5000);
    TEST_ASSERT_EQUAL(1, rc);
