...
static cJSON* assert_result(cJSON* resp) {
    TEST_ASSERT_NULL_MESSAGE(cJSON_GetObjectItem(resp, "error"),
...
void test_ping_returns_pong_true(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(cJSON_IsTrue(cJSON_GetObjectItem(r, "pong")), "pong should be true");
    cJSON_Delete(resp);
}
...
void test_set_auth_token_missing_token_field_returns_invalid_params(void) {
    cJSON* resp =
        dispatch_and_parse("{\"jsonrpc\":\"2.0\",\"id\":15,\"method\":\"bramble.setAuthToken\","
                           "\"params\":{}}");
    TEST_ASSERT_EQUAL_INT(RPC_ERR_INVALID_PARAMS, assert_error_code(resp));
    cJSON_Delete(resp);
}
...
void test_set_auth_token_numeric_value_returns_invalid_params(void) {
    cJSON* resp =
        dispatch_and_parse("{\"jsonrpc\":\"2.0\",\"id\":16,\"method\":\"bramble.setAuthToken\","
                           "\"params\":{\"token\":42}}");
    TEST_ASSERT_EQUAL_INT(RPC_ERR_INVALID_PARAMS, assert_error_code(resp));
    cJSON_Delete(resp);
}
...
void test_set_auth_token_boolean_value_returns_invalid_params(void) {
    cJSON* resp =
        dispatch_and_parse("{\"jsonrpc\":\"2.0\",\"id\":17,\"method\":\"bramble.setAuthToken\","
                           "\"params\":{\"token\":true}}");
    TEST_ASSERT_EQUAL_INT(RPC_ERR_INVALID_PARAMS, assert_error_code(resp));
    cJSON_Delete(resp);
}
...
void test_set_auth_token_empty_string_clears_token(void) {
    ...
    cJSON* resp =
        dispatch_and_parse("{\"jsonrpc\":\"2.0\",\"id\":20,\"method\":\"bramble.setAuthToken\","
                           "\"params\":{\"token\":\"\"}}");
    cJSON* r = assert_result(resp);
    TEST_ASSERT_TRUE(cJSON_IsTrue(cJSON_GetObjectItem(r, "ok")));
    cJSON_Delete(resp);
