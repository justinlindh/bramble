...
- `nRF52840 build` (`quality.yml`): the bare-metal nRF52840 target (`nrf/`, bare FreeRTOS + nrfx, no ESP-IDF) built with `arm-none-eabi-gcc` on a hosted runner (the pod image does not carry the ARM embedded toolchain). The post-link step runs `nrf/scripts/size_report.py`, which fails the build when the image's RAM demand exceeds the 200KB budget on the 256KB chip, so the memory gate is inseparable from the build. Gates on `firmware`, `nrf`, or `ci_core`: the target compiles `components/` sources directly
