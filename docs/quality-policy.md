...
- `Static checks` (one pod, `firmware-quality.yml`) runs, as named steps:
  ...
  - Board matrix coverage (`bash scripts/lint/check-board-matrix.sh`: the `board-build-smoke` matrix in `quality.yml`, the `is_board()` list in `scripts/flash.sh`, and the build loop in `scripts/flash-fleet.sh` must each list exactly the boards `scripts/ci-build-firmware.sh` releases)
  ...
  - ARM toolchain version pins (`bash scripts/lint/check-arm-gcc-version.sh`: every reference to the nRF52840 cross-compiler must match `.arm-gcc-version`, and the CI assert and the nRF build must read that file rather than restate the version). This pin is functional rather than cosmetic: the `nRF52840 build` job's memory gate fails on a byte count, GCC releases produce different byte counts from identical source, and the difference exceeds the headroom the T1000-E build runs at, so an unpinned local toolchain yields a size number that looks like CI's and is not. Matching the version is necessary but not sufficient for byte-exact agreement, since the bundled newlib differs between distributions of the same GCC; `nrf/README.md` documents the container that reproduces CI's counts, and CI remains the authority on them
  - Strict shellcheck (`bash scripts/lint/run-shellcheck.sh --strict`)
  - Ruff baseline profile (`bash scripts/lint/run-ruff.sh`): the wrapper holds the pinned ruff version and the rule profile for both `make ci-quality-ruff` and this job. It runs the ruff on PATH when that matches the pin, which in CI is the one baked into the runner image (issue #222). In CI a missing or mismatched ruff fails the step; outside CI the wrapper fetches the pin via `uvx` instead
