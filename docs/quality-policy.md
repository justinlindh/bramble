...
- `nRF52840 build` (`quality.yml`): the bare-metal nRF52840 target (`nrf/`, bare FreeRTOS + nrfx, no ESP-IDF) built with `arm-none-eabi-gcc` on a hosted runner (the pod image does not carry the ARM embedded toolchain). The post-link step runs `nrf/scripts/size_report.py`, which fails the build on any of three limits on the 256KB chip: total RAM demand over 252KB, static (non-heap) RAM over 100KB, or the FreeRTOS heap below its 144KB floor. The floor is what stops new statics being paid for by shrinking the heap, which would move the shortfall to runtime. The memory gate is inseparable from the build. Gates on `firmware`, `nrf`, or `ci_core`: the target compiles `components/` sources directly
