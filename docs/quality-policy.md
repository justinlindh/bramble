...
- `Static checks` (one pod, `firmware-quality.yml`) runs, as named steps:
  ...
  - Ruff baseline profile (`bash scripts/lint/run-ruff.sh`): the wrapper holds the pinned ruff version and the scanned profile, so `make ci-quality-ruff` and this job cannot drift apart. The runner image bakes the pinned ruff (issue #222); the script asserts the PATH binary matches and fails loud on drift, and `RUFF_USE_UVX=1` fetches the same pin via `uvx` on a box with no ruff installed
