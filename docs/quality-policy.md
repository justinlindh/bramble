...
- `Docker build (webapp)`, `(simulator)`, `(firmware-builder)` (`quality.yml`, issue #195): one context per covered Dockerfile, each a `docker build` on the runner host, never pushed. Same step-level area-gate pattern as `Board build smoke`: `webapp` gates on `webapp`; `simulator` gates on `simulator` or `firmware`; `firmware-builder` gates on `firmware` (which covers `docker/firmware-builder/`). Makes Dependabot's docker-ecosystem bumps self-verifying instead of merging on unrelated green checks with nothing having built the image. The fourth shipped Dockerfile, `emulator/Dockerfile`, is not a leg yet: its ESP-IDF stage needs live component-registry egress the runner pool lacks, so a hermetic (vendored/baked) build is tracked in #231 before its leg and required context are added.
