...
| **Reliability model** | Optional ACKs. Basic retries. No flow control. No congestion awareness. | ACK mechanism. Basic retry logic. No formal congestion management. | Three tiers (Broadcast/Normal/Critical). Exponential backoff retries with jitter, end-to-end ACKs, delivery receipts with relay path. Sliding-window flow control is designed and component-tested but not yet wired into the live mesh path; congestion signaling was removed unshipped (admission control is the budget-gated TX path). |
