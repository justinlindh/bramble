...
- Response fields: `uptime_s` (number), `free_heap` (number), `heap` (object: `internal_free`, `internal_largest_free_block`, `internal_min_ever_free`, `dma_free`, `dma_largest_free_block`, `psram_free`, `psram_min_ever_free`), `task_stack_hwm` (array of `{task, hwm_words, hwm_bytes}`), `backpressure` (object: `flood_relay_drops`, `probe_ingress`), `radio_health` (object: `supported`, `tx_power_dbm`, and when supported `device_errors`, `device_errors_str`, `pa_ramp_error`, `status`, `chip_mode`, `cmd_status`, `ocp`, `ocp_expected`, `ocp_ok`, `pa_duty_cycle`, `pa_hp_max`, `pa_rated_dbm`), and on a GPS-capable board `gps_rx_bytes`, `gps_rx_lines`, `gps_chip`, `gps_rx_overruns`, `gps_rx_errors`, `gps_rx_disabled`, `gps_rx_rearm_fail`.
- Example:
