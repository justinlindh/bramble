...
bounded by construction rather than by operator discipline: three announce
rounds, one answer per member per roll-call, one delivery receipt per member
per roll-call, and a member-side cap on how many roll-calls a node will answer
in an hour.

Wire sizes, at the shipped envelope (28-byte DATA prefix + 12-byte nonce +
8-byte channel header + payload + 16-byte tag) and, for the receipt, at
`DELIVERY_RECEIPT_MIN_SIZE` plus one path hop:
...
- **N answers, not 3N.** A member answers a given roll-call at most once, no
  matter how many rounds it hears. Each answer costs one transmission per hop
  back to the initiator, plus the ACK the reliability layer already sends.
- **N delivery receipts, not 3N.** An announce is an ordinary broadcast, so a
  member confirms it through the same broadcast delivery-receipt machinery
  every other broadcast uses, and that is where a ledger row's relay path
  comes from. A receipt is a `0xFFFFFFFF` flood carrying up to
  `BROADCAST_RECEIPT_RETRY_COUNT` (3) transmissions, and the receipt policy
  emits one on any mesh up to 40 peers, so it is not a rounding error next to
  the answer it accompanies. Only the round on which a member first takes the
  roll-call on is confirmed: rounds 2 and 3 cost an already-answered member a
  decode and nothing more, which is what keeps this term N rather than 3N.

So a roll-call's airtime is roughly N answers of 137 bytes times their hop
counts, plus N receipt floods of 40 bytes times their retries, plus 3
announce floods. Answers dominate on frame size, receipts on transmission
count, and neither term is negligible: a roll-call over a 20-node mesh
averaging 2 hops is roughly 40 answer transmissions, up to 60 receipt
transmissions and 3 floods. That is affordable occasionally and ruinous
continuously, which is why both ends are rate limited.

Measured in the simulator, on the 4-node line of
`simulator/gosim/rollcall_test.go` with one member partitioned, a whole
roll-call (three announce rounds, two answers, two confirmations) accounts
for 4977ms of DATA airtime and 3120ms of receipt airtime. These are
simulation results, not bench measurements.
...
| Tracked responders | 24 | Counted in `overflow` rather than dropped silently, so a ledger that could not hold the whole mesh says so. An attested answer outranks a relay path: when the table is full it reclaims a row that carries only a path, so `overflow` can only ever mean "more members answered than this ledger holds" |
...
| Answers a member emits | 12 per rolling hour, whoever asks | Counted in `answer_limited` and reported by that node; the announce is not confirmed either, so an over-asked mesh spends nothing on it |

Both limits are enforced in the firmware, not in the client: they have to
hold for a script driving the RPC directly.

The two limits answer different questions. The initiation interval bounds
what THIS node asks of the fleet. The answer budget bounds what the fleet
will spend on being asked, by anyone: a network-key insider holding the
public channel can broadcast a fresh announce as fast as it likes, and
without a member-side bound each one would cost every node a signature, a
137-byte unicast with its retry ladder and a flooded receipt. The per-node
TX budget would not save the fleet from that, it would be the casualty: the
roll-call answers would consume the budget and legitimate chat, beacons and
receipts would be denied by the lane the roll-call filled. Twelve per hour is
two initiators running continuously at their own five-minute floor, so a
cooperative fleet never meets it.

A refused answer takes no answer-once claim, so the next announce round of
the same roll-call is answered if the budget has room by then.
...
**A relay path is not attested either.** It comes from the broadcast
delivery-receipt machinery, which is authenticated by the shared network key
alone, and a receipt's source address is whatever its sender wrote. A receipt
may therefore only open a ledger row for an address this node already pins,
and even then the row says "a receipt named this member", never "this member
answered". Read the path as a hint about how the announce travelled and the
signature as the evidence.
...
counters (`unattested`, `overflow`, `late`, `pending_dropped`,
`answer_limited`). Times are
...
- `test/test_rollcall.c` covers the pure core against the shipped codecs: the
  ...
  behalf, the stagger bounds, the round schedule, both rate limits across the
  millisecond rollover, the release of a claim whose answer never reached the
  air, and the ledger's idempotence, close, overflow and missing-set rules,
  including a table filled with receipt-supplied paths that still records
  every attested answer and a forged receipt that cannot open a row at all.
