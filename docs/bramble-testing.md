...
**Run:**
```bash
cd /home/justin/.openclaw/workspace/bramble-go
go test ./...
```
...
**Run:**
```bash
cd /home/justin/.openclaw/workspace/bramble/webapp
npx vitest run
```
...
# Mixed
python3 scripts/e2e-test.py ws://192.168.1.64/ws serial:/dev/ttyUSB0
```

**Board addresses:**
- Board 1: `6E1EE666` — local VM `/dev/ttyUSB0`, WiFi `192.168.1.64`
- Board 2: `63929F02` — GPU box `/dev/ttyUSB0`, WiFi `192.168.1.21`
...
```bash
# 1. Core unit tests
cd /home/justin/.openclaw/workspace/bramble && bash test/run_all_tests.sh

# 2. Go SDK tests
cd /home/justin/.openclaw/workspace/bramble-go && go test ./...

# 3. Webapp tests
cd /home/justin/.openclaw/workspace/bramble/webapp && npx vitest run

# 4. Simulator smoke test
cd /home/justin/.openclaw/workspace/bramble/simulator && bash test-e2e.sh
```
