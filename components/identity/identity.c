...
bool identity_check_collision(const bramble_identity_t *my_id,
    ...
    if (my_id->address != beacon_src_addr) return false;
...
int identity_load(bramble_identity_t *id) {
    ...
    size_t len = 32;
    if (nvs_get_blob(h, "priv", id->private_key, &len) != ESP_OK) { nvs_close(h); return -1; }
    len = 32;
    if (nvs_get_blob(h, "pub", id->public_key, &len) != ESP_OK) { nvs_close(h); return -1; }
    nvs_close(h);
...
int identity_generate_and_save(bramble_identity_t *id) {
    if (crypto_generate_identity(id) != 0) return -1;
