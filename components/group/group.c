...
int group_derive_key(const char* name, const uint32_t* sorted_members, int count, uint8_t* key_out,
    ...
    int ret = crypto_hkdf_sha256((const uint8_t*)salt, strlen(salt), (const uint8_t*)sorted_members,
                                 (size_t)count * sizeof(uint32_t), (const uint8_t*)name,
                                 strlen(name), okm, sizeof(okm));
    if (ret != 0)
        return -1;

    if (key_out)
        memcpy(key_out, okm, GROUP_KEY_SIZE);
    if (id_out)
        memcpy(id_out, okm + GROUP_KEY_SIZE, GROUP_ID_SIZE);
...
int group_create(group_manager_t* mgr, const char* name, uint32_t creator_addr,
    ...
    if (!mgr || !name || num_members < 0)
        return -1;
    if (mgr->count >= GROUP_MAX_GROUPS)
        return -1;
    if (num_members + 1 > GROUP_MAX_MEMBERS)
        return -1; /* +creator */
    ...
    for (int i = 0; i < num_members; i++) {
        ...
        if (member_addrs[i] == creator_addr)
            continue;
...
int group_advance_epoch(bramble_group_t* group) {
    ...
    if (crypto_hkdf_sha256((const uint8_t*)salt, strlen(salt), group->group_key, GROUP_KEY_SIZE,
                           info, 2, new_key, GROUP_KEY_SIZE) != 0) {
        return -1;
    }
...
int group_invite_serialize(const bramble_group_t* group, const uint8_t* sender_private_key,
    ...
    if (!group || !sender_private_key || !recipient_public_key || !buf ||
        buf_len < GROUP_INVITE_SIZE)
        return -1;
    ...
    if (crypto_generate_identity(&eph) != 0)
        return -1;
    ...
    if (crypto_hkdf_sha256((const uint8_t*)hkdf_salt, strlen(hkdf_salt), shared_secret,
                           BRAMBLE_KEY_SIZE, group->group_id, GROUP_ID_SIZE, enc_key,
                           BRAMBLE_KEY_SIZE) != 0)
        return -1;
    ...
    if (crypto_aes256gcm_encrypt(enc_key, nonce, group->group_key, GROUP_KEY_SIZE, group->group_id,
                                 GROUP_ID_SIZE, encrypted_key, tag) != 0)
        return -1;

    /* Serialize */
    size_t off = 0;
    memcpy(buf + off, group->group_id, GROUP_ID_SIZE);
    off += GROUP_ID_SIZE;
    memcpy(buf + off, eph.public_key, BRAMBLE_KEY_SIZE);
    off += BRAMBLE_KEY_SIZE;
    memcpy(buf + off, encrypted_key, GROUP_KEY_SIZE);
    off += GROUP_KEY_SIZE;
    memcpy(buf + off, tag, BRAMBLE_TAG_SIZE);
    off += BRAMBLE_TAG_SIZE;
    memcpy(buf + off, group->name, GROUP_NAME_MAX);
    off += GROUP_NAME_MAX;
...
int group_invite_deserialize(const uint8_t* buf, size_t len, const uint8_t* recipient_private_key,
                             ...
                             uint16_t* epoch_out) {
    if (!buf || !recipient_private_key || len < GROUP_INVITE_SIZE)
        return -1;

    size_t off = 0;

    /* Parse fields */
    const uint8_t* group_id = buf + off;
    off += GROUP_ID_SIZE;
    const uint8_t* ephemeral_pub = buf + off;
    off += BRAMBLE_KEY_SIZE;
    const uint8_t* encrypted_key = buf + off;
    off += GROUP_KEY_SIZE;
    ...
    if (crypto_hkdf_sha256((const uint8_t*)hkdf_salt, strlen(hkdf_salt), shared_secret,
                           BRAMBLE_KEY_SIZE, group_id, GROUP_ID_SIZE, dec_key,
                           BRAMBLE_KEY_SIZE) != 0)
        return -1;
    ...
    if (crypto_aes256gcm_decrypt(dec_key, nonce, encrypted_key, GROUP_KEY_SIZE, group_id,
                                 GROUP_ID_SIZE, tag, decrypted_key) != 0)
        return -1;

    /* Output */
    if (group_id_out)
        memcpy(group_id_out, group_id, GROUP_ID_SIZE);
    if (key_out)
        memcpy(key_out, decrypted_key, GROUP_KEY_SIZE);
    if (name_out) {
        memcpy(name_out, name, GROUP_NAME_MAX);
        name_out[GROUP_NAME_MAX - 1] = '\0';
    }
    if (epoch_out)
        *epoch_out = (uint16_t)((buf[off] << 8) | buf[off + 1]);
