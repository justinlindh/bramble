...
int discovery_start(pending_discovery_table_t *table, uint32_t dest_addr, uint32_t query_id, uint32_t now_ms) {
    if (table->count >= MAX_PENDING_DISCOVERIES) return -1;
...
bool discovery_should_retry(const pending_discovery_t *d, uint32_t now_ms) {
    if (d->attempts >= MAX_RREQ_ATTEMPTS) return false;
...
bramble_rreq_t rreq_build_originator(uint32_t my_addr, uint32_t dest_addr, uint32_t query_id, uint32_t encrypted_source) {
    bramble_rreq_t r;
    memset(&r, 0, sizeof(r));
...
bramble_rreq_t rreq_forward(const bramble_rreq_t *incoming, uint32_t my_addr, int8_t rx_rssi, int8_t rx_snr) {
