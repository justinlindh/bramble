...
data_rx_decision_t data_rx_decide(uint32_t dest_addr, uint32_t self_addr, uint32_t src_addr,
    ...
    /* Wire v4: learn a route back to this DATA's originator via the verified
     * last radio hop. See forwarding.h's doc comment for the full rationale;
     * skip the two self-referential cases, and skip broadcast DATA entirely:
     * a broadcast implies no unicast return path worth learning, and
     * installing off it lets a single forged broadcast poison the whole
