...
void sx1262_hard_reset(void) {
    ...
    ESP_LOGW(TAG, "SX1262 hard reset complete, BUSY=%d", gpio_get_level(s_board->radio.busy));
...
int sx1262_set_dio_irq_params(uint16_t irq_mask, uint16_t dio1_mask, uint16_t dio2_mask,
                              uint16_t dio3_mask) {
    uint8_t data[8] = {
        (uint8_t)(irq_mask >> 8),    (uint8_t)(irq_mask & 0xFF),  (uint8_t)(dio1_mask >> 8),
        (uint8_t)(dio1_mask & 0xFF), (uint8_t)(dio2_mask >> 8),   (uint8_t)(dio2_mask & 0xFF),
...
int sx1262_set_cad(void) { return sx1262_write_command(SX1262_CMD_SET_CAD, NULL, 0); }
...
int sx1262_set_dio3_as_tcxo(float voltage, uint32_t timeout_ms) {
    ...
    ESP_LOGD(TAG, "SetDIO3 TCXO: %.1fV (code 0x%02x), timeout %" PRIu32 " ms", voltage, volt_code,
             timeout_ms);
...
int sx1262_calibrate_image(float freq_mhz) {
    ...
    if (freq_mhz >= 902.0f && freq_mhz <= 928.0f) {
        data[0] = 0xE1;
        data[1] = 0xE9; /* 902-928 MHz */
    } else if (freq_mhz >= 863.0f && freq_mhz <= 870.0f) {
        data[0] = 0xD7;
        data[1] = 0xDB; /* 863-870 MHz */
...
int sx1262_init(void) {
    ...
    if (s_board->radio_osc == RADIO_OSC_TCXO_DIO3) {
        ...
        if (sx1262_set_dio3_as_tcxo(s_board->radio_tcxo_voltage, 32) != 0)
            return -1;
    } else {
    ...
    if (sx1262_set_regulator_mode(reg_mode) != 0)
        return -1;

    /* --- LoRa packet type --- */
    if (sx1262_set_packet_type(1) != 0)
        return -1;

    /* --- Buffer base addresses: TX=0, RX=128 --- */
    if (sx1262_set_buffer_base_address(0, 128) != 0)
        return -1;
