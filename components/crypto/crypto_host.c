...
uint32_t crypto_derive_pubkey_hash(const uint8_t *public_key) {
    uint8_t hash[32];
    crypto_sha256(public_key, BRAMBLE_KEY_SIZE, hash);
    return ((uint32_t)hash[4] << 24) | ((uint32_t)hash[5] << 16) |
...
int crypto_x25519_dh(const uint8_t *private_key, const uint8_t *peer_public_key, uint8_t *shared_secret) {
    ...
    if (!priv || !pub) { EVP_PKEY_free(priv); EVP_PKEY_free(pub); return -1; }

    EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new(priv, NULL);
    int ret = -1;
    if (ctx && EVP_PKEY_derive_init(ctx) == 1 && EVP_PKEY_derive_set_peer(ctx, pub) == 1) {
        size_t len = 32;
        if (EVP_PKEY_derive(ctx, shared_secret, &len) == 1) ret = 0;
    }
...
int crypto_hkdf_sha256(const uint8_t *salt, size_t salt_len,
    ...
    if (EVP_PKEY_derive_init(ctx) == 1 &&
        ...
        size_t len = okm_len;
        if (EVP_PKEY_derive(ctx, okm, &len) == 1) ret = 0;
    }
    EVP_PKEY_CTX_free(ctx);
    return ret;
}
...
int crypto_aes256gcm_encrypt(const uint8_t *key, const uint8_t *nonce,
    ...
    if (EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) == 1 &&
        ...
        if (aad && aad_len > 0) EVP_EncryptUpdate(ctx, NULL, &len, aad, (int)aad_len);
...
int crypto_aes256gcm_decrypt(const uint8_t *key, const uint8_t *nonce,
    ...
    if (EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) == 1 &&
        ...
        if (aad && aad_len > 0) EVP_DecryptUpdate(ctx, NULL, &len, aad, (int)aad_len);
...
int crypto_hmac_sha256(const uint8_t *key, size_t key_len,
    ...
    unsigned int len = 32;
    if (HMAC(EVP_sha256(), key, (int)key_len, data, data_len, mac, &len) == NULL) return -1;
    return 0;
}
