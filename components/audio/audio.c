...
static const tone_note_t tone_boot[] = {
    {523, 60, 20}, {659, 60, 20}, {784, 60, 20}, {1047, 120, 0}};
...
static void play_beep_internal(uint16_t freq_hz, uint16_t duration_ms) {
    ...
    while (written < total_samples) {
        size_t chunk =
            (total_samples - written > BUFFER_SAMPLES) ? BUFFER_SAMPLES : (total_samples - written);

        for (size_t i = 0; i < chunk; i++) {
            float t = (float)(written + i) / SAMPLE_RATE;
            buffer[i] = (int16_t)(vol_scale * 16000.0f * sinf(2.0f * (float)M_PI * freq_hz * t));
        }

        size_t bytes_written;
        i2s_channel_write(s_audio.tx_chan, buffer, chunk * sizeof(int16_t), &bytes_written,
                          portMAX_DELAY);
        written += chunk;
    }
...
int audio_init(void) {
    ...
    ESP_LOGI(TAG, "Audio ready: WS=%d BCK=%d DOUT=%d", cfg->audio.i2s_ws, cfg->audio.i2s_bck,
             cfg->audio.i2s_dout);
    return 0;
}
...
bool audio_is_playing(void) { return s_audio.playing; }

bool audio_is_available(void) { return (board_get_config()->capabilities & BOARD_CAP_AUDIO) != 0; }
...
uint8_t audio_get_volume(void) { return s_audio.volume; }
...
bool audio_get_muted(void) { return s_audio.muted; }
