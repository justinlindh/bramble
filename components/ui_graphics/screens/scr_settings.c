...
static void identity_qr_close(void) {
    if (!s_identity_qr_overlay) return;
...
static void identity_qr_open_cb(lv_event_t *e) {
    ...
    if (base64url_encode_no_pad(pubkey, sizeof(pubkey), pubkey_b64url, sizeof(pubkey_b64url)) == 0) {
        ESP_LOGW(TAG, "Failed to encode pubkey for QR share");
        return;
    }

    char share[192];
    snprintf(share, sizeof(share), "bramble://node/v1?n=%s&a=%08lX&pk=%s",
             name_sanitized, (unsigned long)addr, pubkey_b64url);
...
static void radio_load_nvs_config(radio_config_t *cfg) {
    ...
    nvs_handle_t nvs;
    if (nvs_open("bramble_radio", NVS_READONLY, &nvs) != ESP_OK) return;
...
static void radio_save_and_apply(void) {
    if (!s_radio_tx_slider || !s_radio_sf_dd || !s_radio_bw_dd || !s_radio_cr_dd) return;
    ...
    ESP_LOGI(TAG, "Radio config saved: SF%u BW%lu TX%d CR4/%u",
             cfg.sf, (unsigned long)cfg.bw_hz, cfg.tx_power, cfg.coding_rate);
}
...
static void build_radio_config_section(lv_obj_t *cont, lv_group_t *g) {
    ...
    if (g) lv_group_add_obj(g, s_radio_tx_slider);
    ...
    if (cfg.sf >= 7 && cfg.sf <= 12) lv_dropdown_set_selected(s_radio_sf_dd, cfg.sf - 7);
    if (g) lv_group_add_obj(g, s_radio_sf_dd);
    ...
    uint16_t bw_idx = 0;
    if (cfg.bw_hz == 250000) bw_idx = 1;
    else if (cfg.bw_hz == 500000) bw_idx = 2;
    lv_dropdown_set_selected(s_radio_bw_dd, bw_idx);
    if (g) lv_group_add_obj(g, s_radio_bw_dd);
    ...
    if (g) lv_group_add_obj(g, s_radio_cr_dd);
...
static void channel_refresh_list(void) {
    if (!s_channel_section_cont) return;
...
static void build_channel_manager_section(lv_obj_t *cont, lv_group_t *g) {
    ...
    for (int i = 0; i < num_channels; i++) {
        ...
        lv_obj_t *lbl = lv_label_create(row);
        char ch_text[48];
        snprintf(ch_text, sizeof(ch_text), "#%d %s%s%s",
                 i, name ? name : "?",
                 has_psk ? " " LV_SYMBOL_EYE_CLOSE : "",
                 is_default ? " *" : "");
        ...
        if (!is_default) {
            ...
            lv_obj_add_event_cb(def_btn, channel_set_default_cb, LV_EVENT_CLICKED,
                                (void *)(intptr_t)i);
            if (g) lv_group_add_obj(g, def_btn);
        }
        ...
        if (!is_default) {
            ...
            lv_obj_add_event_cb(rm_btn, channel_remove_cb, LV_EVENT_CLICKED,
                                (void *)(intptr_t)i);
            if (g) lv_group_add_obj(g, rm_btn);
        }
    }
    ...
    if (g) lv_group_add_obj(g, add_btn);
}
...
void scr_settings_create(bramble_layout_t *layout) {
    ...
    if (g) lv_group_add_obj(g, identity_qr_row);
