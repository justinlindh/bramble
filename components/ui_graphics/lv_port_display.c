...
static void flush_cb(lv_display_t* disp, const lv_area_t* area, uint8_t* px_map) {
    display_flush_area(area->x1, area->y1, area->x2, area->y2, (const uint16_t*)px_map);
...
lv_display_t* lv_port_display_init(void) {
    ...
    ESP_LOGI(TAG, "LVGL display port initialized (%dx%d, %d-line buffers)", DISP_HOR_RES,
             DISP_VER_RES, BUF_LINES);
    return disp;
}
