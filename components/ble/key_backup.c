...
backup_state_t key_backup_get_state(const key_backup_ctx_t* ctx) { return ctx->state; }
...
int key_backup_export(key_backup_ctx_t* ctx, const uint8_t* identity_privkey,
                      ...
                      const uint8_t* encryption_key, uint8_t* out, size_t out_max,
                      size_t* out_len) {
    if (ctx->state != BACKUP_AUTHORIZED)
        return -1;
    if (out_max < BACKUP_BLOB_SIZE)
        return -1;
    ...
    if (crypto_random(nonce, NONCE_SIZE) != 0)
        return -1;
    if (crypto_aes256gcm_encrypt(encryption_key, nonce, plaintext, BACKUP_KEY_MATERIAL_SIZE, NULL,
                                 0, ciphertext, tag) != 0)
        return -1;
#else
    ...
    if (EVP_EncryptInit_ex(evp_ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) != 1)
        goto cleanup;
    if (EVP_EncryptInit_ex(evp_ctx, NULL, NULL, encryption_key, nonce) != 1)
        goto cleanup;
    if (EVP_EncryptUpdate(evp_ctx, ciphertext, &outl, plaintext, BACKUP_KEY_MATERIAL_SIZE) != 1)
        goto cleanup;
    if (EVP_EncryptFinal_ex(evp_ctx, ciphertext + outl, &outl) != 1)
        goto cleanup;
    if (EVP_CIPHER_CTX_ctrl(evp_ctx, EVP_CTRL_GCM_GET_TAG, TAG_SIZE, tag) != 1)
        goto cleanup;
...
int key_backup_import(const uint8_t* encrypted_backup, size_t backup_len,
                      ...
                      uint32_t* addr_out) {
    if (backup_len != BACKUP_BLOB_SIZE)
        return -1;
...
#ifdef ESP_PLATFORM
    // Use project crypto component (works on ESP32 via mbedtls backend)
    if (crypto_aes256gcm_decrypt(encryption_key, nonce, ciphertext, BACKUP_KEY_MATERIAL_SIZE, NULL,
                                 0, tag, plaintext) != 0)
        return -1;
#else
    // Host path: use OpenSSL for tests
    ...
    if (EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) != 1)
        goto cleanup;
    if (EVP_DecryptInit_ex(ctx, NULL, NULL, encryption_key, nonce) != 1)
        goto cleanup;
    if (EVP_DecryptUpdate(ctx, plaintext, &outl, ciphertext, BACKUP_KEY_MATERIAL_SIZE) != 1)
        goto cleanup;
