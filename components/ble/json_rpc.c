...
rpc_method_t rpc_method_from_string(const char* method) {
    if (strcmp(method, "get_config") == 0)
        return RPC_METHOD_GET_CONFIG;
    if (strcmp(method, "set_config") == 0)
        return RPC_METHOD_SET_CONFIG;
    if (strcmp(method, "send_message") == 0)
        return RPC_METHOD_SEND_MESSAGE;
    if (strcmp(method, "get_nodes") == 0)
        return RPC_METHOD_GET_NODES;
    if (strcmp(method, "get_routes") == 0)
        return RPC_METHOD_GET_ROUTES;
    if (strcmp(method, "get_status") == 0)
        return RPC_METHOD_GET_STATUS;
    if (strcmp(method, "get_messages") == 0)
        return RPC_METHOD_GET_MESSAGES;
...
int rpc_build_response(const rpc_response_t* resp, char* buf, size_t buf_len) {
    ...
    return snprintf(buf, buf_len, "{\"jsonrpc\":\"2.0\",\"result\":%s,\"id\":%d}", resp->result,
                    resp->id);
}
