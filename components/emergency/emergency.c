...
int emergency_activate(emergency_manager_t* mgr, int32_t lat_e7, int32_t lon_e7, int16_t alt_m,
                       uint8_t battery, const char* msg, uint32_t now_ms) {
    ...
    if (msg && msg[0]) {
        ...
        if (len > EMERGENCY_SHORT_MSG_LEN)
            len = EMERGENCY_SHORT_MSG_LEN;
...
int emergency_beacon_deserialize(const uint8_t* buf, size_t len, emergency_beacon_t* beacon) {
    if (len < EMERGENCY_BEACON_MIN_SIZE)
        return -1;
    ...
    if (beacon->msg_len > EMERGENCY_SHORT_MSG_LEN)
        return -1;
    if (len < (size_t)(EMERGENCY_BEACON_MIN_SIZE + beacon->msg_len))
        return -1;
...
int emergency_cancel_serialize(const emergency_cancel_t* cancel, uint8_t* buf, size_t buf_len) {
    if (buf_len < EMERGENCY_CANCEL_SIZE)
        return -1;
...
int emergency_record_received(emergency_manager_t* mgr, const emergency_beacon_t* beacon,
                              uint32_t now_ms) {
