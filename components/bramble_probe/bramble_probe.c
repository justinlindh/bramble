...
static void rate_limit_init(probe_rate_limit_t *rl, uint8_t max_tokens, uint32_t refill_ms, uint32_t now_ms) {
...
static void rate_limit_refill(probe_rate_limit_t *rl, uint32_t now_ms) {
    if (rl->refill_interval_ms == 0) return;
...
void bramble_probe_init(bramble_probe_state_t *state, uint32_t my_addr,
    ...
    rate_limit_init(&state->send_limiter, PROBE_RATE_LIMIT_TOKENS,
                    PROBE_RATE_LIMIT_REFILL_MS, 0);
}
...
void bramble_probe_tick(bramble_probe_state_t *state, uint32_t now_ms) {
    ...
    if (state->pending_ack.active && now_ms >= state->pending_ack.send_at_ms) {
        if (state->send_fn)
            state->send_fn((const uint8_t *)&state->pending_ack.ack,
                           state->pending_ack.ack_len, state->send_ctx);
    ...
    if (state->collecting &&
        (now_ms - state->result.start_ms) >= PROBE_COLLECTION_WINDOW_MS) {
