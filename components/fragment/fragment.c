...
int fragment_split(const uint8_t *plaintext, size_t pt_len, uint16_t message_id,
    ...
    if (pt_len == 0 || !plaintext || !frags_out) return -1;

    int num_frags = (int)((pt_len + FRAG_MAX_PLAINTEXT - 1) / FRAG_MAX_PLAINTEXT);
    if (num_frags > FRAG_MAX_FRAGMENTS || num_frags > max_frags) return -1;

    for (int i = 0; i < num_frags; i++) {
        size_t offset = (size_t)i * FRAG_MAX_PLAINTEXT;
        size_t chunk = pt_len - offset;
        if (chunk > FRAG_MAX_PLAINTEXT) chunk = FRAG_MAX_PLAINTEXT;
...
int reassembly_add(reassembly_ctx_t *ctx, const frag_header_t *hdr,
    ...
    if (frag_len > FRAG_MAX_PLAINTEXT) return -1;
    ...
    if (slot->received_mask & bit) return 0;
    ...
    if (slot->received_mask == full_mask) return 1;
    return 0;
}
