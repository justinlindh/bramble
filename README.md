...
The firmware runs on real hardware (including T-Deck Plus and Heltec V3/V4 targets), with host-side testing used to validate protocol behavior continuously during development.
...
- **End-to-end direct messages.** DMs use per-peer AES-256-GCM sessions keyed by a role-symmetric quad-DH X25519 exchange (`components/dm_session`), and are not readable by other channel members. Each message has forward secrecy from a symmetric HKDF ratchet, with a coarse per-epoch DH ratchet for post-compromise recovery. An identity-bound 7-digit safety number (SAS) lets two people verify a contact out of band; the verification UX ships in the web client, the T-Deck graphical build, and on the e-paper pager itself (a per-peer view under the Nodes screen with a two-step confirm), and verified state is remembered per contact and cleared automatically if a contact's identity key changes. See [docs/SECURITY-MODEL.md](docs/SECURITY-MODEL.md) for the threat model and residuals.
- **Channel encryption + multi-hop channels.** Channel messages use AES-256-GCM (AEAD) with the channel ID hidden inside the ciphertext, and they relay multi-hop via the flood relay.
...
- **Attested roll-call.** An initiator broadcasts a roll-call and every member answers with an Ed25519 identity-signed unicast, building a per-member ledger with relay paths (`bramble.startRollCall` / `bramble.getRollCall`, with a panel in the web client). An anchored mesh audits against the endorsed member set; an un-anchored mesh reports observed responders only. A signature proves an answer and absence proves nothing; both initiation and answering are rate-limited because a roll-call costs N unicast answers. Verified by host tests and the collision-modeled simulator; no field deployment ([docs/rollcall.md](docs/rollcall.md)).
- **Deployment digital twin.** Each node exports its observed topology over RPC (`bramble.exportTopology`), and the simulator's `twin` subcommand replays that deployment against the collision model: capacity (delivery rate vs offered load) and which single node's loss partitions the mesh. The twin replays observed links at a moment in time; it predicts nothing about unplaced nodes, and its results are simulation ([docs/digital-twin.md](docs/digital-twin.md)).
- **Browser playground.** `make playground` boots the actual firmware binary as a small virtual fleet on a simulated ether, with a guided tour through provisioning (including the fail-closed inert state), multi-hop relay, SAS verification, and a delivery receipt. First contact needs no hardware and no toolchain; a browser fleet is not a field deployment ([docs/playground.md](docs/playground.md)).
- **Store-and-forward mailbox:** offline nodes receive queued messages when they rejoin the mesh.
- **Location sharing with privacy tiers:** presence, zone (a 0.003° by 0.006° cell, about 334 m north-south and 668 m east-west at the equator), or exact; per-peer control over what you share and with whom. Location payloads are AES-256-GCM encrypted end to end and padded to a fixed size so the ciphertext does not leak which tier was chosen; timing and the fact that a node shares location remain observable ([docs/SECURITY-MODEL.md](docs/SECURITY-MODEL.md)).
...
| Seeed Wio-WM1110 Dev Kit | nRF52840 | None (BLE-only interface) | None | LR1110 | N/A | Experimental: joins the bench mesh, BLE RPC + provisioning over an encrypted link, identity/key/bonds persist in flash; GNSS and power management not yet implemented ([nrf/](nrf/)) |
| Seeed SenseCAP T1000-E (card tracker) | nRF52840 | None (BLE-only interface) | Button | LR1110 | Buzzer | Experimental: boots, BLE RPC, joins the mesh; GNSS driver implemented for the onboard AG3335 module and bench-verified (2026-07-31); power management not yet implemented ([nrf/](nrf/)) |
...
See [simulator/README.md](simulator/README.md) for setup and scenarios. For measured scale results under the collision model, [docs/results/simulation-2026-07-honest-baseline.md](docs/results/simulation-2026-07-honest-baseline.md) is the source of truth (it supersedes the June numbers for planning; the earlier collision-free "100% at 200 nodes" figures were retracted as sim artifacts). In short, re-measured on 2026-07-24 at the PHY firmware actually transmits on (the frequency plan's SF9/125 kHz, not the radio profile's SF10): 30% message delivery at 10 nodes, 10% at 50, 0-5% at 100 and 0% at 200, as a single channel saturates under control-plane load (0.91 erlang offered by control traffic alone at 50 nodes, 1.94 at 200). Scale is bounded by radio profile, node density, and hop budget; there is no field-tested-at-scale result.
