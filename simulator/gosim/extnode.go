...
// for a stock node adoption confirms rather than corrects the price. It is what
// prices a node whose sf/bw/cr differ from the plan default (an NVS override,
// another region's build, or a non-default coding rate), and it keeps the ether
// following the firmware either way.
//
// Pricing a frame at the wrong SF is not a rounding error: a 60-byte frame is
// 386 ms at SF9 and 731 ms at SF10, close to double. Against the emulator
// scenarios' budget-exempt short beacon cadence (EMU_BEACON_INTERVAL_MS,
// 2.6-4.1 s) the higher figure puts a 3-node cell at about 89% of channel
// capacity, where the half-duplex + any-overlap-collision model turns the wait
// for a receiver's first clean beacon into a heavy-tailed lottery instead of a
// bounded time.
//
// Learning the PHY from the node rather than restating it in each scenario is
// what keeps the two coupled: a frequency-plan or profile change moves the
// ether with the firmware, automatically. A scenario that pins radio.sf or
// radio.bw_hz still owns its PHY and is never overridden.
